# om garafisch gui te maken
import tkinter as tk

class WordPuzzleApp:
    def __init__(self, root):
        self.root = root
        # correct zin bewerken 
        ...
        # Titel van de game
        tk.Label(root, text="Bouw de juiste zin", font=("Arial", 16, "bold"),
                 bg=bg_kleur).pack(pady=5)

        # Live invoer
        self.output = tk.Label(root, text="", font=("Arial", 12), bg="white",
                               width=30, height=2, relief="sunken")
        self.output.pack(pady=5)

        #  # Lijst met beschikbare woorden (waarvan sommige niet bij de juiste zin horen)
        woorden = [
        ...
        for index, woord in enumerate(woorden):
            btn = tk.Button(knop_frame, text=woord, font=("Arial", 10),
                            width=8, height=1,
                            bg="white", fg="black",
        ...
        tk.Button(controle_frame, text="🔄 Opnieuw", font=("Arial", 10),
                  width=12, command=self.reset).grid(row=0, column=0, padx=5)
    ...
    def remove_last_word(self):
        if self.input:
            self.input.pop()
            self.update_display()
    # om lijst en en zin leeg te maken
    def reset(self):
        self.input = []
        self.update_display()
    #om de zin in de gui te werken 
